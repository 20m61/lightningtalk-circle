...
router.put('/profile', [
  ...
  body('expertise.*').optional().isString().isLength({ max: 50 }).withMessage('Each expertise must be less than 50 characters'),
...
router.post('/talks', [
  ...
  body('description').isString().trim().isLength({ min: 20, max: 2000 }).withMessage('Description must be 20-2000 characters'),
  body('abstract').optional().isString().isLength({ max: 500 }).withMessage('Abstract must be less than 500 characters'),
  body('eventId').isString().notEmpty().withMessage('Event ID is required'),
  body('duration').optional().isInt({ min: 5, max: 60 }).withMessage('Duration must be 5-60 minutes'),
  body('tags').optional().isArray().withMessage('Tags must be an array'),
  body('tags.*').optional().isString().isLength({ max: 30 }).withMessage('Each tag must be less than 30 characters')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
 ...
 *                 format: binary
 */
router.post('/talks/:talkId/presentation', 
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    // Get voting data
    const votingData = await Promise.all(talks.map(async (talk) => {
      const votes = await req.app.locals.database.query('votes', { talkId: talk.id });
      ...
      return {
        ...
        averageRating: votes && votes.length > 0
          ? votes.reduce((sum, vote) => sum + vote.rating, 0) / votes.length
          : 0,
    ...
    const analytics = {
      ...
      averageRating: votingData.length > 0
...
router.post('/practice-timer', [
  ...
  try {
    ...
    talk.practiceData.averageDuration = talk.practiceData.totalDuration / talk.practiceData.totalSessions;
...
router.get('/feedback/:talkId', [
  ...
  try {
    ...
    const aggregatedFeedback = {
      ...
      ratings: {
        content: feedback ? feedback.filter(f => f.ratings?.content).map(f => f.ratings.content) : [],
        delivery: feedback ? feedback.filter(f => f.ratings?.delivery).map(f => f.ratings.delivery) : [],
        engagement: feedback ? feedback.filter(f => f.ratings?.engagement).map(f => f.ratings.engagement) : []
      },
      comments: [
    ...
    aggregatedFeedback.averageRatings = {
      content: aggregatedFeedback.ratings.content.length > 0
        ? aggregatedFeedback.ratings.content.reduce((a, b) => a + b, 0) / aggregatedFeedback.ratings.content.length
        : 0,
      delivery: aggregatedFeedback.ratings.delivery.length > 0
        ? aggregatedFeedback.ratings.delivery.reduce((a, b) => a + b, 0) / aggregatedFeedback.ratings.delivery.length
        : 0,
      engagement: aggregatedFeedback.ratings.engagement.length > 0
        ? aggregatedFeedback.ratings.engagement.reduce((a, b) => a + b, 0) / aggregatedFeedback.ratings.engagement.length
        : 0
    };
