...
router.get('/search', async (req, res) => {
  try {
    ...
    if (dateFrom && isNaN(Date.parse(dateFrom))) {
      return res.status(400).json({
        error: 'Validation failed',
        details: [{ field: 'dateFrom', message: 'dateFrom must be a valid ISO 8601 date' }]
      });
    }
    if (dateTo && isNaN(Date.parse(dateTo))) {
      return res.status(400).json({
        error: 'Validation failed',
        details: [{ field: 'dateTo', message: 'dateTo must be a valid ISO 8601 date' }]
      });
    }
    ...
    const paginatedEvents = events.slice(offset, offset + perPageNum);
  ...
  try {
    ...
    const enhancedAnalytics = {
      ...
      timeline: {
        ...
        peakRegistrationDay: analytics.peakRegistrationDay || null
      },
      demographics: {
