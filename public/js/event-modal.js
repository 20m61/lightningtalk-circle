...
class EventModal {
  ...
  bindEvents() {
    ...
    this.modal.querySelector('.action-calendar').addEventListener('click', () => this.handleCalendar());
  ...
  populateModal(eventData) {
    ...
    this.modal.querySelector('.event-venue').textContent = 
      eventData.venue || '会場調整中';
    this.modal.querySelector('.event-participants').textContent = 
      `${eventData.participants?.total || 0}名 (定員: ${eventData.capacity || '∞'})`;
    this.modal.querySelector('.event-format-detail').textContent = 
      this.getFormatDetail(eventData.format);
    
    // Details section
    this.modal.querySelector('.event-description').innerHTML = 
      eventData.description || '<p>詳細情報は準備中です。</p>';
    this.modal.querySelector('.event-schedule').innerHTML = 
      this.formatSchedule(eventData.schedule);
    this.modal.querySelector('.event-notes').innerHTML = 
      eventData.notes || '<p>特になし</p>';
    
    // Participation section
    this.modal.querySelector('.online-count').textContent = 
      eventData.participants?.online || 0;
    this.modal.querySelector('.onsite-count').textContent = 
      eventData.participants?.onsite || 0;
  ...
  updateActionButtons(eventData) {
    const registerButtons = this.modal.querySelectorAll('.action-register, .action-register-footer');
  ...
  handleSurvey() {
    if (this.currentEvent) {
      ...
      window.dispatchEvent(new CustomEvent('openSurvey', {
        detail: { event: this.currentEvent }
      }));
    }
  }
  ...
  announceModal(action) {
    ...
    announcement.textContent = action === 'open' ? 
      'イベント詳細モーダルが開きました' : 
      'イベント詳細モーダルが閉じました';
