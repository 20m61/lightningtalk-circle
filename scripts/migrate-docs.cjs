...
class DocumentMigrator {
  ...
  migrateDirectory(sourcePath, targetPath) {
    ...
    if (this.dryRun) {
      console.log(colors.yellow(`🔍 [DRY RUN] Would migrate directory: ${sourcePath} → ${targetPath}`));
      return;
    }
    ...
    try {
      ...
      console.error(colors.red(`❌ Directory migration failed for ${sourcePath}: ${error.message}`));
    }
  }
...
`;
    ...
    if (fs.existsSync(troubleshootingDir)) {
      ...
      const troubleshootingPath = path.join(this.projectRoot, 'docs-new/quick-start/04-troubleshooting.md');
