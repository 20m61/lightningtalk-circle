...
class RollbackTester {
  ...
  async testRollbackCommand() {
    ...
    try {
      ...
      const { success } = await this.runCommand('node scripts/migrate-to-dynamodb.js status', false);
...
PORT=3001
      ...
      this.logResult('Config Rollback', isRolledBack, 
  ...
  async testRollbackTime() {
    ...
    try {
      ...
      this.logResult('Rollback Time', withinLimit, 
  ...
  async testHealthCheck() {
    ...
    try {
      ...
      if (success && stdout) {
        ...
        this.logResult('Health Check', isHealthy, 
